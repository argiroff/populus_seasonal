...
hill_aov_rep <- function(x) {
  
  tmp1 <- aov(hill ~ year * month + Error(genotype / (year * month)), data = x)
...
#### Hill list anova, repeated ####

get_hill_div_aov_rep_list <- function(x) {
...
bc_hill_div_aov_rep <- map(bc_hill_div, .f = get_hill_div_aov_rep_list)
...
### Hill diversity ANOVA, Table S3  

```{r functions_hill_div_pvalues_and_tableS3}
...
```{r hill_div_pvalues_and_tableS3}
# Table S3 input, 16S
tableS3_input_16S <- get_hill_div_aov_tables(bc_hill_div_aov_summary[1:3]) %>% 
...
# Table S3 input
tableS3_input <- get_hill_div_aov_tables(bc_hill_div_aov_summary[4:6]) %>% 
  
  bind_rows(.id = "Plant habitat") %>% 
  
  mutate(Community = "Fungi") %>% 
  
  bind_rows(tableS3_input_16S, .) %>% 
...
plot_hill_div2 <- function(hill.div, cdl.stats, stat.labels.P, plot.title, hill.index, community) {
  ...
  tmp1 <- ggplot() + 
    ...
    scale_y_continuous(expand = expansion(mult = c(0.15, 0.15)), limits = c(0, NA)) +
...
```{r fig2_alphadiv}
# Fig. 2a data (bacteria/archaea)
bc_hill_div_data_fig2a <- prep_hill_div_data(bc_hill_div[1:3], "hill1")

# Fig. 2b data (fungi)
bc_hill_div_data_fig2b <- prep_hill_div_data(bc_hill_div[4:6], "hill1")

# Fig. 2a CLD (bacteria/archaea)
bc_hill_div_cld_fig2a <- prep_hill_div_cld(bc_hill_div_seasonality_cld[1:3], "hill1")

# Fig. 2b CLD (fungi)
bc_hill_div_cld_fig2b <- prep_hill_div_cld(bc_hill_div_seasonality_cld[4:6], "hill1")

# Fig. 2a P (bacteria/archaea)
bc_hill_div_pval_fig2a <- prep_hill_div_cld(bc_hill_div_seasonality_Pval[1:3], "hill1")

# Fig. 2b P (fungi)
bc_hill_div_pval_fig2b <- prep_hill_div_cld(bc_hill_div_seasonality_Pval[4:6], "hill1")

# Fig 2a
fig2a <- plot_hill_div2(
  
  bc_hill_div_data_fig2a, 
  
  bc_hill_div_cld_fig2a, 
  
  bc_hill_div_pval_fig2a, 
...
# Fig 2b
fig2b <- plot_hill_div2(
  
  bc_hill_div_data_fig2b, 
  
  bc_hill_div_cld_fig2b, 
  
  bc_hill_div_pval_fig2b, 
...
# Construct figure
fig2 <- plot_grid(fig2_grid, fig2_legend, ncol = 1, rel_heights = c(1, .033))
...
ggsave2(
  ...
  filename = "data/results/fig2.tiff", 
...
ggsave2(
  ...
  filename = "data/results/fig2.eps", 
...
## Environmental time series, Fig. S1a  

```{r env_time_series_figS1a}
# Get sample day
...
env_pca_points <- as.data.frame(env_pca_summary$sites) %>% 
  ...
  mutate(
    ...
    season = map_chr(month, .f = get_season),
...
figS1b <- ggplot() + 
  ...
  geom_point(
    ...
    aes(x = pc1_mean, y = pc2_mean, fill = season, shape = year), 
...
ggsave2(
  ...
  filename = "data/results/figS1.png", 
...
ggsave2(
  ...
  filename = "data/results/figS1.pdf", 
...
permanova_rep <- function(dist.mat, env.table) {
...
# PERMANOVA, repeated
# bc_permanova_rep <- map2(bc_dist, bc_metadata_habsplit_filtered, .f = permanova_rep)
# 
# write_rds(
# 
#   bc_permanova_rep,
# 
#   file = "data/working/bc_permanova_rep.rds"
# 
# )

bc_permanova_rep <- read_rds(
...
```{r tableS1}
# Format table input
tableS1a_input <- get_permanova_table(bc_permanova) %>% 
  
  mutate(`Plant habitat` = map_chr(`Plant habitat`, .f = plant_hab))

tableS1b_input <- get_permanova_table(bc_permanova_rep) %>% 
...
write_tsv(
  ...
  file = "data/results/tableS1a.txt"
...
write_tsv(
  ...
  file = "data/results/tableS1b.txt"
...
run_dbRDA <- function(asv.tab, metadata.tab) {
  ...
  tmp4 <- tibble(
    ...
    sample_unique_id = rownames(tmp1)
    
  ) %>%
    
    inner_join(., tmp3, by = "sample_unique_id") %>%
    
    inner_join(., tmp2, by = "seasonality") %>%
    
    arrange(match(sample_unique_id, rownames(tmp1)))
  
  tmp5 <- capscale(

    tmp1 ~ host_species + year + month + pc1_mean + pc2_mean,
...
run_anova_dbRDA <- function(x) {
  
  tmp1 <- anova.cca(
    
    x, permutations = 999, by = "terms", model = "full"
...
bc_dbRDA <- map2(
  ...
  set_names(nm = names(bc_metadata_habsplit_filtered))
...
```{r dbRDA_fig1_functions}
#### dbRDA eigenvalues ####

get_dbRDA_axis_contributions <- function(x) {
  
  tmp1 <- 100 * (x$CCA$eig / sum(x$CCA$eig))
...
get_dbRDA_loadings <- function(dbrda.ord, metadata.table) {
  
  tmp1 <- scores(dbrda.ord)
  ...
  tmp3 <- tmp1$biplot %>%
    ...
    filter(variable %in% c("pc1_mean", "pc2_mean")) %>%
    
    mutate(
      
      variable = ifelse(variable == "pc1_mean", "PC1", "PC2")
      
    )
  
  tmp4 <- list(tmp2, tmp3) %>%
    
    set_names(nm = c("sites", "biplot"))
...
get_dbRDA_centroids <- function(fig.data) {
  
  tmp1 <- fig.data$sites %>% 
    
    group_by(genotype, year, month, seasonality) %>%
    
    summarise(
      
      mean_cent1 = mean(CAP1), 
      
      mean_cent2 = mean(CAP2), 
      
      ebar_min1 = mean(CAP1) - se(CAP1), 
      
      ebar_max1 = mean(CAP1) + se(CAP1), 
      
      ebar_min2 = mean(CAP2) - se(CAP2), 
      
      ebar_max2 = mean(CAP2) + se(CAP2)
...
plot_dbRDA <- function(fig.data, biplot.data, axis.contributions, fig.title1, fig.title2, stat.labels.P) {
  
  tmp0 <- biplot.data$biplot %>%
    
    mutate(
      
      cap1_new = ifelse(CAP1 >= 0, (CAP1 * 4) + 0.1, (CAP1 * 4) - 0.1),
      
      cap2_new = ifelse(CAP2 >= 0, (CAP2 * 4) + 0.1, (CAP2 * 4) - 0.1)
  ...
  tmp3 <- ggplot() + 
    ...
    geom_hline(yintercept = 0, linetype = 2, colour = "black", linewidth = 0.5) +
    ...
    geom_segment(
      ...
      aes(x = 0, y = 0, xend = CAP1 * 3.75, yend = CAP2 * 3.75),
    ...
    geom_text(
      ...
      aes(x = cap1_new, y = cap2_new, label = variable), 
    ...
    geom_text(
      ...
      aes(x = -Inf, y = Inf, vjust = 1.3, hjust = -0.25, label = pval_label),
    ...
    labs(
      ...
      title = bquote(bold(.(fig.title1)) ~~~ .(fig.title2)), 
      
      x = bquote('dbRDA 1 (' * .(tmp1) * '% of fitted variation)'),
      
      y = bquote('dbRDA 2 (' * .(tmp2) * '% of fitted variation)')
...
```{r dbRDA_fig1}
# dbRDA axis contributions
bc_dbRDA_eig <- map(bc_dbRDA, .f = get_dbRDA_axis_contributions)

# dbRDA sample loadings
bc_dbRDA_loadings <- map2(bc_dbRDA, bc_metadata_habsplit_filtered, .f = get_dbRDA_loadings)

# dbRDA loading centroids
bc_dbRDA_centroids <- map(bc_dbRDA_loadings, .f = get_dbRDA_centroids)

# Get February time points
feb_dates <- bc_dbRDA_centroids$bc_16s_rh %>% 
  
  select(genotype, year, month, season, seasonality, everything()) %>%
  
  mutate(across(!c(genotype, year, month, seasonality, season), ~replace(., is.numeric(.), NA))) %>% 
  
  filter(seasonality %in% c("Feb 2018", "Feb 2019")) %>% 
  
  distinct()

# Add February time points to 16S LE data
bc_dbRDA_centroids$bc_16s_le <- bc_dbRDA_centroids$bc_16s_le %>% 
...
bc_dbRDA_centroids$bc_its_le <- bc_dbRDA_centroids$bc_its_le %>% 
...
dbRDA_fig_titles1 <- c("A", "C", "E", "B", "D", "F")

dbRDA_fig_titles2 <- c("Bacteria/archaea, leaf end.", "Bacteria/archaea, root end.", "Bacteria/archaea, rhizo.", 
                       "Fungi, leaf end.", "Fungi, root end.", "Fungi, rhizo.")
...
bc_dbRDA_figs <- pmap(
  ...
  list(
    ...
    bc_dbRDA_centroids, bc_dbRDA_loadings, bc_dbRDA_eig, 
    
    dbRDA_fig_titles1, dbRDA_fig_titles2, bc_permanova_seasonality_pval
...
```{r dbRDA_construct_fig1}
# Get shared legend
fig1_legend <- get_legend(
  
  bc_dbRDA_figs$bc_16s_le +
...
fig1_grid <- plot_grid(
  ...
  bc_dbRDA_figs$bc_16s_le + theme(legend.position = "none"), 
  
  bc_dbRDA_figs$bc_its_le + theme(legend.position = "none"), 
  
  bc_dbRDA_figs$bc_16s_re + theme(legend.position = "none"), 
  
  bc_dbRDA_figs$bc_its_re + theme(legend.position = "none"), 
  
  bc_dbRDA_figs$bc_16s_rh + theme(legend.position = "none"), 
  
  bc_dbRDA_figs$bc_its_rh + theme(legend.position = "none"), 
...
ggsave2(
  ...
  filename = "data/results/fig1.tiff", 
...
ggsave2(
  ...
  filename = "data/results/fig1.eps", 
...
```{r dbrda_env_r2_functions}
#### Get env r2 ####

get_env_r2 <- function(x) {
...
## ASV and sample totals and seqs  

```{r functions_seq_totals}
#### Get sequence totals for non-subsampled dataset by habitat ####

get_habsplit_seq_total <- function(x) {
  ...
  tmp1 <- x %>%
    ...
    summarise(sum_n_seqs = sum(n_seqs)) %>%
    
    ungroup(.) %>%
    
    summarise(
      
      min_n_seqs = min(sum_n_seqs),
      
      median_n_seqs = median(sum_n_seqs),
      
      max_n_seqs = max(sum_n_seqs)
...
```{r seq_totals}
# Create list of minimum sample counts
bc_sample_seq_cutoffs2 <- list(0, 0, 0, 0, 0, 0) %>% 
  
  set_names(nm = c("bc_16s_le_sample_seq_min", "bc_16s_re_sample_seq_min", "bc_16s_rh_sample_seq_min", "bc_its_le_sample_seq_min", "bc_its_re_sample_seq_min", "bc_its_rh_sample_seq_min"))

bc_sample_seq_min2 <- map2(bc_sample_counts, bc_sample_seq_cutoffs2, .f = min_sample_n_seqs)

# Split asv tables by habitat
bc_asv_habsplit_df_SRS_input2 <- bc_asv_filtered %>% 
  ...
  # Drop samples below minimum sequence depth
  map2(., bc_sample_seq_cutoffs2, .f = trim_to_seq_min) %>% 
...
# Get medians
bc_sample_medians <- map(
  
  bc_asv_habsplit_df_SRS_input,
  
  .f = get_habsplit_seq_total
  
)
```

### ASV totals subsampled by habitat  

```{r functions_asv_totals}
...
get_community_rate_change_mean <- function(x) {
  ...
  tmp1 <- x %>%
    ...
    group_by(sample_type_1, host_species_1, genotype_1, interval) %>%
...
#### Community rate change LME ####

community_rate_change_lme <- function(x) {
...
# LME
bc_community_rate_change_lme <- map(
  
  bc_community_rate_change_bray_means,
  
  .f = community_rate_change_lme
...
```{r fig3}
# Get fig3
fig3 <- plot_community_rate_change(
...
ggsave2(
  ...
  filename = "data/results/fig3.tiff", 
...
ggsave2(
  ...
  filename = "data/results/fig3.eps", 
...
```{r lm_slope}
# Get input for lm slopes
lm_slopes <- bc_community_rate_change_bray_lm %>% 
...
### Core group ANOVA table, Table S6
...
```{r fig4_heatmap_input_data}
# Format group ANOVA
bc_core_group_heatmap_P <- tableS6_input %>%
  
  mutate(
    
    id = paste(Community, `Plant habitat`, eco_group, sep = "-"),
...
# Format group taxonomic information
bc_core_group_heatmap_tax <- tableS5_input %>%
  
  mutate(
    
    group_id = paste("Group", Group, sep = " "),
    
    community_id = ifelse(Community == "Bacteria/Archaea", "16s", "its"),
    
    planthab_id = NA,
    
    planthab_id = ifelse(`Plant habitat` == "Leaf endosphere", "le", planthab_id),
    
    planthab_id = ifelse(`Plant habitat` == "Root endosphere", "re", planthab_id),
    
    planthab_id = ifelse(`Plant habitat` == "Rhizosphere", "rh", planthab_id)
    
  ) %>%
  
  select(community_id, planthab_id, group_id, Genus, `% of group`) %>%
  
  group_by(community_id, planthab_id, group_id) %>%
  ...
  mutate(
    ...
    Genus = ifelse(Genus == "Nitrososphaeraceae", "Uncl. Nitrososphaeraceae", Genus)
    
  ) %>%
  
  mutate(
    
    tax_label = paste(Genus, " ", "(", group_id, ")", sep = ""),
    
    tax_label = factor(tax_label, levels = tax_label),
    
    id = paste(community_id, planthab_id, group_id, sep = "-")
    
  ) %>%
  
  left_join(., bc_core_group_heatmap_P, by = "id") %>%
...
# Get Z scores
bc_core_group_heatmap_input <- bc_core_groups_relabund %>%
  ...
  group_by(plant_habitat, Community) %>%
  
  mutate(mean_relabund_z = z_score(mean_relabund)) %>%
  
  ungroup() %>%
  
  mutate(
    
    community_id = ifelse(Community == "Bacteria and Archaea", "16s", "its"),
    
    planthab_id = NA,
    
    planthab_id = ifelse(plant_habitat == "Leaf endosphere", "le", planthab_id),
    
    planthab_id = ifelse(plant_habitat == "Root endosphere", "re", planthab_id),
    
    planthab_id = ifelse(plant_habitat == "Rhizosphere", "rh", planthab_id),
    
    id = paste(community_id, planthab_id, eco_group, sep = "-")
    
  ) %>%
  
  select(id, Community, plant_habitat, seasonality, mean_relabund_z) %>%
  
  inner_join(., bc_core_group_heatmap_tax, by = "id") %>%
  
  mutate(
    
    tax_label_final = paste(tax_label, P, sep = ""),
    
    tax_label_final = factor(tax_label_final, levels = unique(tax_label_final)),
    
    plant_habitat = as.character(plant_habitat),
    
    plant_habitat = ifelse(plant_habitat == "Leaf endosphere", "Leaf", plant_habitat),

    plant_habitat = ifelse(plant_habitat == "Root endosphere", "Root", plant_habitat),

    plant_habitat = ifelse(plant_habitat == "Rhizosphere", "Rhizosphere", plant_habitat),

    plant_habitat = factor(plant_habitat, levels = c("Leaf", "Root", "Rhizosphere"))
    
  )
```

```{r fig4_heatmap_functions}
#### Core group heatmap ####

get_core_group_heatmap <- function(z.data, plot.title) {
  ...
  tmp1 <- ggplot() +
    ...
    geom_tile(
      ...
      aes(x = seasonality, y = tax_label_final, fill = mean_relabund_z),
    ...
    scale_fill_viridis(
      ...
      limits = range(bc_core_z_score_range$min_z, bc_core_z_score_range$max_z)
...
```{r fig4_heatmap}
# 4a data
fig4a_input <- bc_core_group_heatmap_input %>%
  
  filter(Community == "Bacteria and Archaea")

# 4b data
fig4b_input <- bc_core_group_heatmap_input %>%
  
  filter(Community == "Fungi")

# Range
bc_core_z_score_range <- list(
  
  min(bc_core_group_heatmap_input$mean_relabund_z),
  
  max(bc_core_group_heatmap_input$mean_relabund_z)
  
) %>%
  
  set_names(nm = c("min_z", "max_z"))

# Fig 4a
fig4a_heatmap <- get_core_group_heatmap(fig4a_input, "A")

# Fig 4b
fig4b_heatmap <- get_core_group_heatmap(fig4b_input, "B")
...
fig4_grid <- plot_grid(
  ...
  fig4a_heatmap + theme(legend.position = "none"), 
  
  fig4b_heatmap + theme(legend.position = "none"),
...
ggsave2(
  ...
  filename = "data/results/fig4.tiff", 
...
ggsave2(
  ...
  filename = "data/results/fig4.eps", 
...
### Core group host species  

```{r core_group_host}
# Get input
core_group_input <- bc_core_groups_relabund %>%
...
# Correlation
core_corr_input <- core_group_input %>%

  select(deltoides, trichocarpa)

core_corr <- corr.test(core_corr_input, method = "pearson")

core_corr_siglabel <- core_corr$r %>%
...
# Construct figure
fig5 <- plot_grid(fig5_grid, fig5_legend, nrow = 1, rel_widths = c(1, 0.1))
...
ggsave2(
  ...
  filename = "data/results/fig5.tiff", 
...
ggsave2(
  ...
  filename = "data/results/fig5.eps", 
...
### Hub Z scores by species  

```{r hub_species_functions}
...
# Correlation
hub_species_corr_input <- bc_important_asv_z_combined_hostspecies %>%
  
  select(deltoides, trichocarpa)

hub_corr <- corr.test(hub_species_corr_input, method = "pearson")

hub_corr_siglabel <- hub_corr$r %>%
...
```{r tableS8}
# Table S8
tableS8_input <- bc_important_asvs %>% 
...
```{r tableS9}
# Table S9
tableS9_input <- get_funguild_aov_tables(bc_funguild_aov_summary) %>%
...
ggsave2(
  ...
  filename = "data/results/fig6.png", 
...
ggsave2(
  ...
  filename = "data/results/fig6.tiff", 
...
ggsave2(
  ...
  filename = "data/results/fig6.eps", 
...
ggsave2(
  ...
  filename = "data/results/fig6.pdf", 
...
```{r nitrospirales}
bc_nitrospirales <- bc_asv_habsplit_df_SRS_output$bc_16s_rh_asv_df %>%
  ...
  inner_join(bc_metadata_habsplit_filtered$bc_16s_rh, ., by = "sample_unique_id")

nitrospirales_aov <- aov(n_seqs ~ host_species * month * year, data = bc_nitrospirales)

nitrospirales_aov_summary <- summary(nitrospirales_aov)

nitrospirales_fig <- ggplot() +
  
  geom_point(
    
    data = bc_nitrospirales,
    
    aes(x = seasonality, y = n_seqs),
...
bc_rh_core_relabund <- bc_core_groups_relabund %>%
  
  filter(Community == "Bacteria and Archaea" & plant_habitat == "Rhizosphere") %>%
  
  select(sample_unique_id, eco_group, group_relabund) %>%
  
  pivot_wider(
    
    id_cols = sample_unique_id,
    
    names_from = "eco_group",
    
    values_from = "group_relabund"
    
  ) %>%
  
  inner_join(., bc_nitrospirales, by = "sample_unique_id") %>%
  
  select(sample_unique_id, `Group 1`, `Group 2`, `Group 3`, `Group 4`, n_seqs) %>%
  
  as.data.frame(.) %>%
  
  column_to_rownames(var = "sample_unique_id")

nitrospirales_corr <- corr.test(bc_rh_core_relabund)

nitrospirales_corr_r <- nitrospirales_corr$r %>%
...
nitrospirales_corr_P <- nitrospirales_corr$p %>%
