...
```{r figS3_alphadiv}
# Fig. S3a data (bacteria/archaea)
bc_hill_div_data_figS3a <- prep_hill_div_data(bc_hill_div[1:3], "hill0")

# Fig. S3b data (fungi)
bc_hill_div_data_figS3b <- prep_hill_div_data(bc_hill_div[4:6], "hill0")

# Fig. S3a CLD (bacteria/archaea)
bc_hill_div_cld_figS3a <- prep_hill_div_cld(bc_hill_div_seasonality_cld[1:3], "hill0")

# Fig. S3b CLD (fungi)
bc_hill_div_cld_figS3b <- prep_hill_div_cld(bc_hill_div_seasonality_cld[4:6], "hill0")

# Fig. S3a P (bacteria/archaea)
bc_hill_div_pval_figS3a <- prep_hill_div_cld(bc_hill_div_seasonality_Pval[1:3], "hill0")

# Fig. S3b P (fungi)
bc_hill_div_pval_figS3b <- prep_hill_div_cld(bc_hill_div_seasonality_Pval[4:6], "hill0")

# Fig S3a
figS3a <- plot_hill_div2(
  
  bc_hill_div_data_figS3a, 
  
  bc_hill_div_cld_figS3a, 
  
  bc_hill_div_pval_figS3a, 
...
# Fig S3b
figS3b <- plot_hill_div2(
  
  bc_hill_div_data_figS3b, 
  
  bc_hill_div_cld_figS3b, 
  
  bc_hill_div_pval_figS3b, 
...
figS3_grid <- plot_grid(
  
  figS3a + theme(legend.position = "none"), 
...
# Construct figure
figS3 <- plot_grid(figS3_grid, figS3_legend, ncol = 1, rel_heights = c(1, .033))
...
```{r figS2_alphadiv}
# Fig. S2a data (bacteria/archaea)
bc_hill_div_data_figS2a <- prep_hill_div_data(bc_hill_div[1:3], "hill1")

# Fig. S2b data (fungi)
bc_hill_div_data_figS2b <- prep_hill_div_data(bc_hill_div[4:6], "hill1")

# Fig. S2a CLD (bacteria/archaea)
bc_hill_div_cld_figS2a <- prep_hill_div_cld(bc_hill_div_seasonality_cld[1:3], "hill1")

# Fig. S2b CLD (fungi)
bc_hill_div_cld_figS2b <- prep_hill_div_cld(bc_hill_div_seasonality_cld[4:6], "hill1")

# Fig. S2a P (bacteria/archaea)
bc_hill_div_pval_figS2a <- prep_hill_div_cld(bc_hill_div_seasonality_Pval[1:3], "hill1")

# Fig. S2b P (fungi)
bc_hill_div_pval_figS2b <- prep_hill_div_cld(bc_hill_div_seasonality_Pval[4:6], "hill1")

# Fig S2a
figS2a <- plot_hill_div2(
  
  bc_hill_div_data_figS2a, 
  
  bc_hill_div_cld_figS2a, 
  
  bc_hill_div_pval_figS2a, 
...
# Fig S2b
figS2b <- plot_hill_div2(
  
  bc_hill_div_data_figS2b, 
  
  bc_hill_div_cld_figS2b, 
  
  bc_hill_div_pval_figS2b, 
...
# Arrange figures in grid
figS2_grid <- plot_grid(
  
  figS2a + theme(legend.position = "none"), 
...
# Construct figure
figS2 <- plot_grid(figS2_grid, figS2_legend, ncol = 1, rel_heights = c(1, .033))
...
```{r figS4_alphadiv}
# Fig. S4a data (bacteria/archaea)
bc_hill_div_data_figS4a <- prep_hill_div_data(bc_hill_div[1:3], "hill2")

# Fig. S4b data (fungi)
bc_hill_div_data_figS4b <- prep_hill_div_data(bc_hill_div[4:6], "hill2")

# Fig. S4a CLD (bacteria/archaea)
bc_hill_div_cld_figS4a <- prep_hill_div_cld(bc_hill_div_seasonality_cld[1:3], "hill2")

# Fig. S4b CLD (fungi)
bc_hill_div_cld_figS4b <- prep_hill_div_cld(bc_hill_div_seasonality_cld[4:6], "hill2")

# Fig. S4a P (bacteria/archaea)
bc_hill_div_pval_figS4a <- prep_hill_div_cld(bc_hill_div_seasonality_Pval[1:3], "hill2")

# Fig. S4b P (fungi)
bc_hill_div_pval_figS4b <- prep_hill_div_cld(bc_hill_div_seasonality_Pval[4:6], "hill2")

# Fig. S4a
figS4a <- plot_hill_div2(
  
  bc_hill_div_data_figS4a, 
  
  bc_hill_div_cld_figS4a, 
  
  bc_hill_div_pval_figS4a, 
...
# Fig. S4b
figS4b <- plot_hill_div2(
  
  bc_hill_div_data_figS4b, 
  
  bc_hill_div_cld_figS4b, 
  
  bc_hill_div_pval_figS4b, 
...
# Construct figure
figS4 <- plot_grid(figS4_grid, figS4_legend, ncol = 1, rel_heights = c(1, .033))
...
## Alpha diversity by host species, Fig. S5  

```{r alpha_div_figS5_functions}
...
```{r figS5_alpha_div}
# Get figS5 input data
figS5_input <- bc_hill_div %>%
...
# Get figure
figS5 <- hill_div_SI_fig(figS5_input)
...
```{r tableS1}
# Format table input
tableS1_input <- get_permanova_table(bc_permanova) %>% 
...
plot_pcoa <- function(fig.data.means, axis.contributions, fig.title1, fig.title2, stat.labels.P) {
...
pcoa_fig_titles1 <- c("A", "C", "E", "B", "D", "F")

pcoa_fig_titles2 <- c("Bacteria/archaea, leaf end.", "Bacteria/archaea, root end.", "Bacteria/archaea, rhizo.", 
...
bc_pcoa_figs <- pmap(
  ...
  list(bc_pcoa_centroids, bc_pcoa_eig, pcoa_fig_titles1, pcoa_fig_titles2, bc_permanova_seasonality_pval), 
...
### ASV totals, Table S11b  

```{r functions_tableS11b_asv_totals}
#### Get ASV totals ####
...
write_tsv(
  ...
  file = "data/results/tableS11b.txt"
  
)
```

### ASV totals subsampled by habitat, table S11c  

```{r functions_table_S11c_asv_totals}
...
write_tsv(
  ...
  file = "data/results/tableS11c.txt"
...
```{r tableS12}
# Table S12 input
tableS12_input <- get_hill_div_aov_overall_tables(bc_hill_div_aov_summary_overall)
...
### Overall Hill diversity, Fig. S16  
...
```{r functions_tableS13_permanova}
#### PERMANOVA overall ####
...
### Overall PERMANOVA results, Table S13  

```{r functions_compile_tableS13}
#### Get PERMANOVA tables for Table S13 ####
...
community_rate_change_gam <- function(x) {
  
  tmp1 <- gam(mean_bray ~ s(interval, k = 4), data = x)
  
  tmp2 <- summary.gam(tmp1)
...
get_community_rate_change_gam_P <- function(x) {
...
# Community rate change GAM
bc_community_rate_change_bray_gam <- map(
  
  bc_community_rate_change_bray_means, 
  
  .f = community_rate_change_gam
  
)

# Community rate change lm P values
bc_community_rate_change_gam_P <- map(
  
  bc_community_rate_change_bray_gam, 
  
  .f = get_community_rate_change_gam_P
...
fig3 <- plot_community_rate_change(
  ...
  bc_community_rate_change_gam_P,
...
### Rate change spp. effects, Fig. S8  

```{r figS8_functions}
#### Plot community rate change ####

plot_community_rate_change_spp <- function(
  ...
  tmp1 <- ggplot() + 
    ...
    geom_smooth(
      ...
      aes(
        ...
        group = interaction(plant_habitat, host_species_1)
...
```{r figS8_rate_change_spp}
# Get input
bc_community_rate_change_bray_means_combined2 <- bc_community_rate_change_bray_means_combined %>%
  
  mutate(host_species_1 = ifelse(host_species_1 == "deltoides", "italic(P.~deltoides)", "italic(P.~trichocarpa)"))

# Get figS8
figS8 <- plot_community_rate_change_spp(
  
  bc_community_rate_change_bray_combined,
  
  bc_community_rate_change_bray_means_combined2
...
## Environmental time series, Fig. S6a  

```{r env_time_series_figS6a}
# Combine data
env_combined <- env %>%
...
community_env_change_means <- community_env_change %>%
  ...
  group_by(community, plant_habitat, genotype_1, seasonality_pairs) %>%
...
fig2 <- ggplot() +
  ...
  geom_point(
    ...
    aes(x = mean_euclidean, y = mean_bray, fill = plant_habitat, colour = plant_habitat),
...
```{r tableS4}
# Get input for Table S4
tableS4_input <- community_env_change_lm %>% 
...
```{r generate_betadisper_plots_figS1}
# Fig. S1a data (bacteria/archaea)
bc_betadisper_data_figS1a <- bc_betadisper_distance[1:3] %>% 
  
  bind_rows()

# Fig. S1b data (fungi)
bc_betadisper_data_figS1b <- bc_betadisper_distance[4:6] %>% 
  
  bind_rows()

# Fig. S1a CLD (bacteria/archaea)
bc_betadisper_cld_figS1a <- get_betadisper_fig_data(bc_betadisper_seasonality_cld[1:3])

# Fig. S1b CLD (fungi)
bc_betadisper_cld_figS1b <- get_betadisper_fig_data(bc_betadisper_seasonality_cld[4:6])

# Fig. S1a P (bacteria/archaea)
bc_betadisper_pval_figS1a <- get_betadisper_fig_data(bc_betadisper_seasonality_Pval[1:3])

# Fig. S1b P (fungi)
bc_betadisper_pval_figS1b <- get_betadisper_fig_data(bc_betadisper_seasonality_Pval[4:6])

# Fig S1a
figS1a <- plot_betadisper2(
  
  bc_betadisper_data_figS1a, 
  
  bc_betadisper_cld_figS1a, 
  
  bc_betadisper_pval_figS1a, 
...
# Fig S1b
figS1b <- plot_betadisper2(
  
  bc_betadisper_data_figS1b, 
  
  bc_betadisper_cld_figS1b, 
  
  bc_betadisper_pval_figS1b, 
...
figS9 <- get_abund_occupancy_fig2(bc_abund_occupancy)
...
# Get figure
figS10 <- get_core_contribution_fig(
...
# Scree plots
figS11 <- get_scree_plots(bc_scree_input)
...
```{r fig4}
# Plot core groups
fig4 <- get_core_group_figs2(bc_core_groups_relabund)
...
### Core group host species, Fig. S12  

```{r figS12}
# Get figS12 input
figS12_input <- bc_core_groups_relabund %>%
...
# Correlation
figS12_corr_input <- figS12_input %>%

  select(deltoides, trichocarpa)

figS12_corr <- corr.test(figS12_corr_input, method = "pearson")

figS12_siglabel <- figS12_corr$r %>%
...
### Hub Z scores by species, Fig. S13  
...
# Correlation
figS13_corr_input <- bc_important_asv_z_combined_hostspecies %>%
  
  select(deltoides, trichocarpa)

figS13_corr <- corr.test(figS13_corr_input, method = "pearson")

figS13_siglabel <- figS13_corr$r %>%
...
# Construct figure
figS18 <- plot_grid(figS18_grid, figS18_legend, ncol = 1, rel_heights = c(1, 0.067))
...
### Guilds by host species, Fig. S14  
...
```{r figS14}  
# Input
figS14_input <- bc_its_funguild_relabund_input %>%
...
figS14 <- funguild_SI_fig(figS14_input)
